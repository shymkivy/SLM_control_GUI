...
if ~isempty(app.UIImagePhaseTableSelection)
    current_pat = unique(app.UIImagePhaseTable.Data.Pattern(app.UIImagePhaseTableSelection(:,1)));
elseif ~isempty(tab_data)
    current_pat = unique(tab_data.Pattern);
else
    current_pat = [];
end
...
for n_pat = 1:numel(current_pat)

    coord = f_sg_mpl_get_coords(app, 'pattern', current_pat(n_pat));
    coord_zero = coord;
    coord_zero.xyzp = [0 0 0];

    power_corr = f_sg_apply_xy_power_corr(reg1.pw_corr_data, coord.xyzp(:,1:2));

    tab_pat = tab_data(tab_data.Pattern == current_pat(n_pat),:);
    ...
    tab_data(tab_data.Pattern == current_pat(n_pat),:) = tab_pat;
