...
System.source = (1/(System.Nx* System.Ny))*ones(System.Nx, System.Ny); %Amplitude at SLM
...
% Load some sample data and format for GPU use if needed.
load('SampleData.mat')
[LX, LY, LZ] = size(TargetIntensity); 
if System.useGPU ==1
    Masks = gpuArray(TargetIntensity);
    NegativeMasks = gpuArray(TargetDarkenss);
else
    Masks = TargetIntensity;
    NegativeMasks = TargetDarkenss;
end
...
for n1 = 1:5
    figure;
    imagesc(NegativeMasks(:,:,n1))
end

%Compute the kernels for propagation at all defined depths
[ HStacks ] = function_Hstacks( System,Depths );

%Compute holograms with Iterative methods superposition, sequential and
%global Gerchberg Saxton
...
% 1 - Binary targets 
Hologram{4} = function_NOVO_CGH_binary( System, HStacks, Masks,Depths,NovoCGHOptions );

%2 - Bright voxels, Dark voxels, and relaxed constraints anywhere where KickMasks = Masks = 0, 
Hologram{5} = function_NOVO_CGH_dualbinary( System, HStacks, Masks, NegativeMasks,Depths,NovoCGHOptions );

%3 - Variable Intensity Holograms with a threshold based cost function
Hologram{6} = function_NOVO_CGH_VarI( System, HStacks, Masks,Depths,NovoCGHOptions);
...
Display{10} = double(NegativeMasks>0);
